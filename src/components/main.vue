  ...
  <section class=" flex flex-row-reverse flex-1 sm:flex-col gap-4">
    ...
    <div class="flex flex-1 flex-col">
      ...
      <div class="stat">
        <div class="stat-title">Отжимания</div>
        <div
          :class="infoALLStages[indexStage + 1][0] <= STATS_USER[0] ? `stat-value text-green-500` : `stat-value text-indigo-500`">
          {{ STATS_USER[0] }}</div>
      </div>
      <div class="stat">
        <div class="stat-title">Подтягивания</div>
        <div
          :class="infoALLStages[indexStage + 1][1] <= STATS_USER[1] ? `stat-value text-green-500` : `stat-value text-indigo-500`">
          {{ STATS_USER[1] }}</div>
      </div>
      <div class="stat">
        <div class="stat-title">Пресс</div>
        <div
          :class="infoALLStages[indexStage + 1][2] <= STATS_USER[2] ? `stat-value text-green-500` : `stat-value text-indigo-500`">
          {{ STATS_USER[2] }}</div>
      </div>
      <div class="stat">
        <div class="stat-title">Приседания</div>
        <div
          :class="infoALLStages[indexStage + 1][3] <= STATS_USER[3] ? `stat-value text-green-500` : `stat-value text-indigo-500`">
          {{ STATS_USER[3] }}</div>
      </div>
    </div>
    </div>
